...
@Component({
  selector: 'app-EditCheapter',
  templateUrl: './EditCheapter.component.html',
  styleUrls: ['./EditCheapter.component.css']
})
export class EditCheapterComponent implements OnInit {
  ...
  ngOnInit() {
    try {
       this.model = localStorage.getItem("Cheapter");
  ...
  onSubmit(formData: any) {
    ...
    if (this.fileToUploadbook !== null) {
      objToSend.append('Note', this.fileToUploadbook,this.fileToUploadbook.name);
    }
    this.http.patch<any>(`${environment.apiUrl}api/Cheapter/UpdateCheapter?cheapterId=${this.model2.id}`, objToSend)
      .subscribe(res =>{
        this.spinner=0;
